...
// ---------------------------------------------------------------------------
// Configuration — MAX STRESS: 1500 concurrent users, 5 minute hold
// ---------------------------------------------------------------------------
...
export const options = {
  scenarios: {
    max_stress: {
      ...
      stages: [
        ...
        { duration: "5s",  target: 5000 },  // overwhelm
        { duration: "1m",  target: 5000 },  // sustain the pain
  ...
  thresholds: {
    http_req_duration: ["p(95)<10000"],  // p95 under 10s (relaxed to find breaking point)
    errors: ["rate<0.50"],               // up to 50% errors (we want to see where it breaks)
...
// No sleep between operations — maximum pressure
// ---------------------------------------------------------------------------
export default function () {
  ...
  // Create a user
  const email = `max_${__VU}_${__ITER}_${Date.now()}@t.co`;
  res = http.post(
    `${BASE_URL}/users`,
  ...
  res = http.post(
    `${BASE_URL}/urls`,
    JSON.stringify({
      original_url: `https://example.com/max-${__VU}-${__ITER}-${Date.now()}`,
      title: `MAX URL ${__VU}`,
      user_id: userId,
    }),
    { headers }
